...
export default function IDELayout() {
  ...
  const {
    openFilePicker,
    openFolderPicker
  } = useLocalFileImport(createFile, updateFileContent, openFile, rootFolderId);
  const {
    isRunning,
    output,
    executeCode,
  ...
  return <div className={`h-screen w-screen flex flex-col overflow-hidden ${isDark ? 'dark' : ''}`}>
      ...
      <div className="h-10 bg-sidebar border-b border-sidebar-border flex items-center justify-between px-4 shrink-0">
        <div className="flex items-center gap-2 text-primary-foreground">
          ...
          <span className="text-sm font-bold tracking-tight text-primary-foreground">CodePiler</span>
        </div>
        <div className="flex items-center gap-1 text-primary">
          ...
          <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setTheme(isDark ? 'light' : 'vs-dark')}>
              ...
              <ResizablePanel defaultSize={65}>
                <div className="h-full flex flex-col">
                  ...
                  <div className="flex-1">
                    {activeFile ? <CodeEditor value={activeContent} language={LANGUAGE_CONFIG[activeLang]?.monacoLang || activeLang} theme={theme} onChange={val => updateFileContent(activeFileId!, val)} /> : <div className="h-full flex items-center justify-center text-muted-foreground text-sm">
